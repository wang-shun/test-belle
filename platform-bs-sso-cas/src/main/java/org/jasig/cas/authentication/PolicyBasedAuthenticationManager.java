...
public class PolicyBasedAuthenticationManager implements AuthenticationManager {
	 ...
	 */
	private final static String ACCOUNT_LOCKED_STATUS = "2";

	/**
	 * ACCOUNT_STATUS_FIELD
	 */
	private final static String ACCOUNT_STATUS_FIELD = "state";
    ...
    public final Authentication authenticate(final Credential... credentials) throws AuthenticationException, CustomAccountLockedException{
		...
		// 判断账户是否被锁定
		if (attributes != null && attributes.get(ACCOUNT_STATUS_FIELD) != null
				&& ACCOUNT_LOCKED_STATUS.equals((String) attributes.get(ACCOUNT_STATUS_FIELD))) {
			throw new CustomAccountLockedException("账户处于锁定状态！");
		}
