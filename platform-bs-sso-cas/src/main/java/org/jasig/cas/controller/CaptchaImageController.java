...
public class CaptchaImageController extends AbstractController {
  ...
  protected ModelAndView handleRequestInternal(HttpServletRequest request,
    ...
    //定义图片尺寸
    int width = 105 * this.captchaType, height = (this.captchaType == 1) ? 38 : 50;
    //创建内存图像
    BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
    //获取图形上下文
    Graphics g = image.getGraphics();
    //设定背景色
    g.setColor(this.getRandColor(random, 0, 180));
    //设定图形的矩形坐标及尺寸
    ...
    if (this.captchaType == 1) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        sRand += rand;
        g.setColor(this.getRandColor(random, 150, 255));
        //将此数字画到图片上
    ...
    //将验证码存入SESSION
    //    request.getSession().setAttribute(CAPTCHA_CODE, sRand);
    //    String random_key = EncryptionUtil.md5Encode(UUID.randomUUID().toString());

    StringBuilder random_key = new StringBuilder();
    random_key.append(CustomConstant.CAPTCHA.RANDOM_KEY);
    random_key.append(UUID.randomUUID().toString());

    //    String random_key = CAPTCHA_CODE + UUID.randomUUID().toString();
    request.getSession().setAttribute(CustomConstant.CAPTCHA.RANDOM_KEY, random_key.toString());

    redisClient.put(random_key.toString(), sRand, 3 * 60L);
    //图像生效
    g.dispose();
