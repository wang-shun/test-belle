...
// 此类定义了一些正则表达式
Reg = function() {
	this.noempty = /^([\s\S]*)$/, this.user = /^\w{6,12}$/;// 用户名 一般是 6-12位
	...
	this.decmal5 = /^(-([1-9]\d*.\d*|0.\d*[1-9]\d*))|0?.0+|0$/;// 非正浮点数（负浮点数 +
																// 0）
...
Fv = function(c) {
	...
	this.reg = function(cf) {// 注册表单,主要是将配置信息注册到表单中
		...
		for ( var i = 0; i < cf.length; i++) {
			...
			var r = (typeof (k.regExp) == "string") ? this.regexp[k.regExp]
					: k.regExp;
			if (o.value == null || o.value == "" || o.type == "select-one") {
				// 添加默认表单组件信息
				k.defaultMsg ? this.addMsg(this.cls.defaultCls, k.defaultMsg,
						k.msgTip) : "";
				// 添加得到焦点时的信息
				k.focusMsg ? this.addFocusListener(o, {
					cls : this.cls.focusCls,
					msg : k.focusMsg,
					tip : k.msgTip
				}) : "";
			} else {
	...
	this.submit = function() {// 由于事件监听函数不能有参数,所以这里用一个闭包
		var f = this.form;// f为this.form 的一个引用，当在闭包函数中的时候，this
	...
	isPassed = function(o, c, r) {// 是否通过验证
		...
		case 'password': {// 判断文本控件，这样的控件是可以有正则表达式的
			...
			// return (!c.allownull && !r.test(o.value)) || (c.allownull &&
			// o.value!='' && !r.test(o.value));
		}
		case 'radio':
	...
	this.onblurMsg = function(o, c) {
		...
		var r = (typeof (c.regExp) == "string") ? this.regexp[c.regExp]
				: c.regExp;
		return function() {
			var clazz, msg;
			if (o.value == "") {
