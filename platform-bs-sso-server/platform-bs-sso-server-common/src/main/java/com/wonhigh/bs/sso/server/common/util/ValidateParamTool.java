...
public class ValidateParamTool
{
	...
	public static boolean validateUserMsgId(String strUserMsgId){
		try{
			if(strUserMsgId!=null&&!"".equals(strUserMsgId) && isSignDigit(strUserMsgId)){
				try
				{
					//转换成整型，如果报错，就越界。
	...
	public static boolean validateSubPort(String strSubPort){
		try{
			if(strSubPort!=null&&!"".equals(strSubPort)&&("*".equals(strSubPort) || isUnSignDigit(strSubPort))&&strSubPort.length()<7){
				return true;
			}else{
				return false;
			}
	...
	public static boolean validateSpNumber(String strSpNumber){
		try
		{
			if(strSpNumber!=null&&!"".equals(strSpNumber)&&("*".equals(strSpNumber)||(isUnSignDigit(strSpNumber)&&strSpNumber.length()<=20))){
				return true;
			}else{
				return false;
			}
	...
	public static boolean validateMobile(String strMobile){
		try{
			if(strMobile != null && !"".equals(strMobile)&&strMobile.length() == 11 && isUnSignDigit(strMobile)){
				return true;
			}else{
				return false;
			}
		...
		try{
			 if(strMobiles != null && !"".equals(strMobiles))
        	 {
        	     ...
        	     if(arrMobiles.length>0&&arrMobiles.length<=100){
            	     ...
            	     {
            	    	 if("".equals(arrMobiles[i])||arrMobiles[i].length() != 11||!isUnSignDigit(arrMobiles[i]))
            	    	 {
            	    		return false;
            	    	 }
            	     }		            	     
